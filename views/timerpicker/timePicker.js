            ...
            return {
                ...
                link: function(scope, elm, attrs) {
                    ...
                    var tem = "<div class='pickerContainer datetimeactive'>" +
                        "<div class='main'>" +
                        "<div  class='header'>" +
                        "<div><span ng-click='cancel()'>取消</span><h2 ng-show='options.title'>{{options.title[0]}}&nbsp;&nbsp;<strong>{{options.title[1]}}</strong>&nbsp;&nbsp;{{options.title[2]}}</h2><span ng-click='ok()'>确定</span></div>" +
                        /* "<div>{{selectDateTime.show}}</div>" +*/
                        "</div>" +
                        "<div class='body'>" +
                        "<div class='row row-no-padding'>" +
                        "<div class='col'  ng-if='!options.hideYear' ><ion-scroll  on-scroll='scrollingEvent(\"year\")' delegate-handle='yearScroll_" + globalId + "' scrollbar-y='false' class='yearContent'>" + "<ul>" + "<li ng-style='year.selected ? { color: \"#43cea9\",fontWeight: \"bold\", fontSize: \"16px\"}:{}' ng-click='selectEvent(\"year\",$index)' ng-repeat='year in yearList'>{{year.text}}</li>" + "</ul>" + "</ion-scroll></div>" +
                        "<div class='col'  ng-if='!options.hideMonth' ><ion-scroll  on-scroll='scrollingEvent(\"month\")' delegate-handle='monthScroll_" + globalId + "' scrollbar-y='false' class='monthContent'>" + "<ul>" + "<li ng-style='month.selected ? { color: \"#43cea9\",fontWeight: \"bold\", fontSize: \"16px\"}:{}' ng-click='selectEvent(\"month\",$index)' ng-repeat='month in monthList'>{{month.text}}</li>" + "</ul>" + "</ion-scroll></div>" +
                        "<div class='col ' ng-if='!options.hideDate' ><ion-scroll on-scroll='scrollingEvent(\"date\")' delegate-handle='dateScroll_" + globalId + "' scrollbar-y='false' class='dateContent'>" + "<ul>" + "<li ng-style='date.selected ? { color: \"#43cea9\",fontWeight: \"bold\", fontSize: \"16px\"}:{}' ng-click='selectEvent(\"date\",$index)' ng-repeat='date in dateList'>{{date.text}}</li>" + "</ul>" + "</ion-scroll></div>" +
                        "<div class='col ' ng-if='!options.hideTime' ><ion-scroll on-scroll='scrollingEvent(\"time\")' delegate-handle='timeScroll_" + globalId + "' scrollbar-y='false' class='timeContent'>" + "<ul>" + "<li ng-style='time.selected ? { color: \"#43cea9\",fontWeight: \"bold\", fontSize: \"16px\"}:{}' ng-click='selectEvent(\"time\",$index)' ng-repeat='time in timeList'>{{time.text}}</li>" + "</ul>" + "</ion-scroll></div>" +
                        "</div>" +
                    ...
                    if(attrs.title && attrs.title.indexOf('|') != -1) {
                        tempTitle = attrs.title.split('|');
                    }
                    var options = {
                        title: tempTitle,
                    ...
                    function initTime(options) {
                        ...
                        //获取默认选择时间
                        var defaultTime = getDefaultSelectTime(options);
                        var defaultItem, defaultIndex;
                        ...
                        for (var i = 0; i < options.timeNum; i++) {
                            ...
                            for (var j = 0; j < 60 / (options.timeSpan); j++) {
                                ...
                                var item = createDateTimeLi(0, top, data, data == defaultTime, data);
                                scope.timeList.push(item);
                                if (data == defaultTime) {
                                    defaultItem = item;
                                    defaultIndex = scope.timeList.length;
                                }
                            }
                        }
                        prependLi(scope.timeList, 3, "e");
                        //设置默认选择
                        scope.selectDateTime.time.item = defaultItem;
                        scope.selectDateTime.time.index = defaultIndex - 1;
                    }
                    ...
                    function setSelectDateTimeShow() {
                        scope.selectDateTime.show = (scope.options.hideYear ? "" : scope.selectDateTime.year.item.text + " ") + (scope.options.hideMoth ? "" : scope.selectDateTime.month.item.text) + (scope.options.hideDate ? "" : scope.selectDateTime.date.item.text + " ") + (scope.options.hideTime ? "" : scope.selectDateTime.time.item.text);
                    }

                    //获取选中的时间结果
                    function getSelectDateTime() {
                        var value = (scope.options.hideYear ? "" : scope.selectDateTime.year.item.data + "-") + (scope.options.hideMoth ? "" : scope.selectDateTime.month.item.data + "-") + (scope.options.hideDate ? "" : scope.selectDateTime.date.item.data + " ") + (scope.options.hideTime ? "" : scope.selectDateTime.time.item.data);
